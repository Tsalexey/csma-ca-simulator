...
#define Node     10       //number of nodes 
...
#define Tack     3    
#define Tout     33   //Tout=Tdata+Tack
#define Trts     3
#define Tcts     3
#define Twait    33    //Twait=Tdata+Tack
...
bool Sensing2(sensore elenco[], int* vectState, int N, FILE* fout1)
{
	...
	for (i = 0; i < N; i++)
	{
		if (vectState[i] == 4)
		{
			sense2 = 1; //c'è un nodo che ha avuto successo a trasmettere l'RTS e si trova in 4.	
			//cout << "Node " << sorg <<" sensed channel BUSY from node "<< elenco[i].ns << endl;
		}
